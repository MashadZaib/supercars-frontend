...
const BookingRequest = ({
  ...
  const [bookingParties, setBookingParties] = React.useState([]); // starts empty
  ...
  const handleSearchBookingParty = async (searchTerm) => {
    try {
      const data = await getBookingParties(searchTerm);
      if (Array.isArray(data) && data.length > 0) {
        setBookingParties(data);
        toast.info(`${data.length} result(s) found`);
      } else {
        setBookingParties([]);
        toast.warning("No matching booking parties found");
      }
    } catch (error) {
...
useEffect(() => {
  // ✅ Run only when a new modal result arrives
  if (modalResult?.title === "Create New Booking Party") {
    const addNewBookingParty = async () => {
      try {
        // The modalResult.data holds your new booking party form data
        const newParty = await createBookingParty(modalResult.data); // ✅ save to backend

        setBookingParties((prev) => [newParty, ...prev]); // add to top of list
        toast.success(`Booking Party "${newParty.name}" added!`);
      } catch (err) {
        console.error("Error creating booking party:", err);
        toast.error("Failed to add Booking Party");
      } finally {
        setShowModal(false);
      }
    };

    addNewBookingParty(); // ✅ Call the async function
  }
}, [modalResult]);
                ...
                <div className="col-md-6">
                  ...
                  <div className="input-group">
                    <Field
                      ...
                      name="bookingParty"
                      validate={(value) => {
                        if (!value) return "Booking party is required";
                        const exists = bookingParties.some(
                          (party) => (party.name || party) === value
                        );
                        if (!exists) return "Invalid booking party selected";
                      }}
                      className={`form-select ${
                    ...
                    >
                      ...
                      {bookingParties.map((party, index) => (
                        <option key={index} value={party.name || party}>
                          {party.name || party}
                        </option>
                      ))}
                    </Field>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (searchTerm) {
                          handleSearchBookingParty(searchTerm);
                        }
                      }}
                    >
