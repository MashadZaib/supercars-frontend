...
import { createCharge, updateCharge } from "../../api/chargesApi";
...
const Charges = ({
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      if (!bookingId) {
        toast.error("❌ Save Booking Request first!");
        setSubmitting(false);
        return;
      }

      for (const ch of values.charges) {
        const payload = {
          booking_request_id: bookingId,
          charge_name: ch.description,
          currency: ch.currency,
          amount: Number(ch.amount),
          quantity: Number(ch.quantity),
          taxable: ch.taxable ? "True" : "False",
          tax_rate: Number(ch.taxRate),
          notes: ch.notes || null,
          invoice_preview_id: values.invoicePreviewId || 1, // temp
        };

        // 🔥 If charge already exists → UPDATE
        if (ch.id) {
          await updateCharge(ch.id, payload);
        }
        // 🔥 If charge does NOT exist → CREATE
        else {
          const res = await createCharge(payload);
          ch.id = res.id; // save returned id for future edits
        }
      }

      toast.success("Charges saved successfully!");
      if (onSubmit) onSubmit(values, true);
    } catch (err) {
      console.error("❌ Charge submit error:", err);
  ...
  const calculateTotals = (charges, discount) => {
    const subtotal = charges.reduce(
      (sum, charge) => sum + calculateChargeTotal(charge),
      0
    );
    const taxableAmount = charges.reduce(
      (sum, charge) =>
        charge.taxable ? sum + calculateChargeTotal(charge) : sum,
      0
    );
    const taxAmount = charges.reduce(
      (sum, charge) => sum + calculateTaxAmount(charge),
      0
    );
      ...
      >
        {({
          ...
          const totals = (() => {
            ...
            values.charges.forEach((ch) => {
              const amount = Number(ch.amount || 0) * Number(ch.quantity || 1);
              subtotal += amount;

              if (ch.taxable) taxable += amount;
              else dutyFree += amount;
            });

            const taxRate = 12; // Japan Consumption Tax 12%
                      ...
                      <div>
                        ...
                        <button
                          ...
                          onClick={() => {
                            const commonCharge =
                              commonCharges[
                                Math.floor(Math.random() * commonCharges.length)
                              ];
                            push({
                              ...
                              <div>
                                <div className="input-group">
                                  ...
                                  <input
                                    ...
                                    value={calculateChargeTotal(charge).toFixed(
                                      2
                                    )}
                                    readOnly
                                  />
                    ...
                    <div className="col-md-12">
                      <div className="payment-summary-section">
                        <div className="payment-summary-item">
                          <h6>Duty-Free Amount</h6>
                          <h4 className="text-primary">
                            ¥{totals.dutyFree.toFixed(2)}
                          </h4>
                        </div>
                        <div className="payment-summary-item">
              ...
              <div className="card mb-4">
                ...
                <div className="card-body">
                  ...
                  <div className="row mb-3">
                    ...
                    <div className="col-md-4">
                      <label
                        htmlFor="bankDetails.accountName"
                        className="form-label"
                      >
                        Account Name
                      </label>
                    ...
                    <div className="col-md-4">
                      <label
                        htmlFor="bankDetails.accountNameKana"
                        className="form-label"
                      >
                        Account Name (Kana)
                      </label>
                    ...
                    <div>
                      ...
                      >
                        {isSubmitting ? (
                          ...
                          <>
                            <i className="fas fa-save me-1"></i> Save Charges &
                            Payment Details
                          </>
                        )}
                      </button>
